...
{
    // What the bell shows, soonest first. The list is written by the console's
    // administrator, on the Admin screen: every notice here is either a window
    // in which a feature is off, or a standalone announcement such as a rate
    // change. Nothing is held twice, so a window and what partners are told
    // about it can never drift apart.
    public static List<Notice> Upcoming => ConsoleAdmin.Bell();
}
